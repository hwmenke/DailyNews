...
def _compute_tf(df: pd.DataFrame, lookback: int):
    ...
    kf = _kama(close, window=10, fast=2, slow=30)
    km = _kama(close, window=20, fast=2, slow=60)
    ...
    dist_hi  = (close / hi.replace(0, np.nan) - 1.0) * 100
...
def _scan_one(sym: str):
    ...
    try:
        ...
        sma20 = hlc3.rolling(20).mean()
        mad20 = hlc3.rolling(20).apply(lambda x: np.abs(x - x.mean()).mean(), raw=True)
        cci_s = (hlc3 - sma20) / (0.015 * mad20.replace(0, np.nan))

        ema12     = close.ewm(span=12, adjust=False).mean()
        ema26     = close.ewm(span=26, adjust=False).mean()
        macd_line = ema12 - ema26
        macd_h    = macd_line - macd_line.ewm(span=9, adjust=False).mean()
        ...
        for period in [10, 20, 50]:
            ...
            if not np.isnan(k_last) and k_last != 0:
                kama_vals[period] = (price / k_last - 1.0) * 100.0
            else:
        ...
        bb_n     = min(20, len(close) - 1)
        bb_mid_s = close.rolling(bb_n).mean()
        bb_std_s = close.rolling(bb_n).std(ddof=0)
        ...
        if trend_score is not None:
            if trend_score >= 2:  signals.append("STRONG_BULL")
            if trend_score <= -2: signals.append("STRONG_BEAR")

        if len(macd_h) >= 2:
            mh_prev = float(macd_h.iloc[-2])
            mh_curr = float(macd_h.iloc[-1])
            if not (np.isnan(mh_prev) or np.isnan(mh_curr)):
                if mh_prev <= 0 and mh_curr > 0: signals.append("MACD_BULL_CROSS")
                if mh_prev >= 0 and mh_curr < 0: signals.append("MACD_BEAR_CROSS")

        if vol_ratio is not None and vol_ratio > 2:
            signals.append("HIGH_VOLUME")

        if bb_pct is not None:
            if bb_pct < 0.05: signals.append("BB_LOWER_BAND")
            if bb_pct > 0.95: signals.append("BB_UPPER_BAND")

...
def _run_strategy(close, kama_fast, kama_slow, trend_score=None, use_trend=False):
    ...
    if strat_ret.empty or strat_ret.std() == 0:
        return {"ann_ret": None, "ann_vol": None, "sharpe": None,
    ...
    return {"ann_ret": _safe(ann_ret), "ann_vol": _safe(ann_vol), "sharpe": _safe(sharpe),
            "max_dd": _safe(max_dd), "n_trades": n_trades, "win_rate": _safe(win_rate)}


def _weekly_equity(close, kama_fast, kama_slow, trend_score=None, use_trend=False):
    fast_above = (kama_fast > kama_slow).astype(int)
    if use_trend and trend_score is not None:
        trend_ok   = (trend_score > 0).astype(int)
        signal_raw = fast_above * trend_ok
    ...
    for date in strat_w.index:
        if date in bh_w.index:
            curve.append({"date": date.strftime("%Y-%m-%d"),
...
def run_optimization(symbol: str) -> dict:
    ...
    benchmark = {"ann_ret": _safe(bh_ann_ret), "ann_vol": _safe(bh_ann_vol),
                 "sharpe": _safe(bh_sharpe), "max_dd": _safe(bh_max_dd)}
    ...
    for fast_p in FAST_PERIODS:
        for slow_p in SLOW_PERIODS:
            ...
            for use_trend in (False, True):
                ...
                metrics = _run_strategy(close, kama_cache[fast_p], kama_cache[slow_p],
                                        trend_score=trend_score, use_trend=use_trend)
                all_results.append({"label": label, "fast": fast_p, "slow": slow_p,
                                    "use_trend": use_trend, **metrics})
    ...
    equity_curve = []
    if best:
        equity_curve = _weekly_equity(close, kama_cache[best["fast"]], kama_cache[best["slow"]],
                                      trend_score=trend_score, use_trend=best["use_trend"])
    ...
    return {"symbol": symbol, "benchmark": benchmark, "top10": top10, "best": best,
            "equity_curve": equity_curve, "heatmap": heatmap, "total_tested": total_tested,
            "fast_periods": FAST_PERIODS, "slow_periods": SLOW_PERIODS}

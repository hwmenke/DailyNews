...
let series = {
    daily:  { candle: null, bb: {}, rsi: {}, macdLine: null, macdSig: null, macdHist: null, trend: null },
    weekly: { candle: null, bb: {}, rsi: {}, macdLine: null, macdSig: null, macdHist: null, trend: null },
};

const C = {
    bb_upper: '#22c55e', bb_middle: '#22c55e', bb_lower: '#22c55e',
    rsi7: '#06b6d4', rsi14: '#f97316', rsi21: '#a855f7',
    macd_line: '#3b82f6', macd_signal: '#ef4444', macd_hist_pos: '#22c55e', macd_hist_neg: '#ef4444',
    trend_pos: '#22c55e', trend_neg: '#ef4444', trend_zero: '#4a5568',
};
...
function baseOpts() {
    return {
        ...
        crosshair: { mode: LWC.CrosshairMode.Normal, vertLine: { color: '#3d4965', labelBackgroundColor: '#1c2230' }, horzLine: { color: '#3d4965', labelBackgroundColor: '#1c2230' } },
        rightPriceScale: { borderColor: '#30363d' },
        timeScale: { borderColor: '#30363d', timeVisible: true, secondsVisible: false, rightOffset: 6, barSpacing: 6, fixLeftEdge: true },
...
function destroyCharts() {
    ['daily', 'weekly'].forEach(freq => {
        ...
        series[freq] = { candle: null, bb: {}, rsi: {}, macdLine: null, macdSig: null, macdHist: null, trend: null };
        Object.values(kamaPeriods).forEach(p => { p[`series_${freq}`] = null; });
    });
}
...
function buildPanel(freq) {
    ...
    charts[freq].main = LWC.createChart(mainEl, { ...baseOpts(), width: mainEl.clientWidth, height: mainEl.clientHeight });
    ...
    Object.values(kamaPeriods).forEach(meta => {
        meta[`series_${freq}`] = charts[freq].main.addLineSeries({ color: meta.color, lineWidth: 1.5, priceLineVisible: false, lastValueVisible: false });
    });

    charts[freq].rsi = LWC.createChart(rsiEl, { ...baseOpts(), width: rsiEl.clientWidth, height: rsiEl.clientHeight, rightPriceScale: { borderColor: '#30363d', autoScale: false, scaleMargins: { top: 0.05, bottom: 0.05 } } });
    ...
    charts[freq].macd = LWC.createChart(macdEl, { ...baseOpts(), width: macdEl.clientWidth, height: macdEl.clientHeight });
    ...
    charts[freq].trend = LWC.createChart(trendEl, { ...baseOpts(), width: trendEl.clientWidth, height: trendEl.clientHeight, rightPriceScale: { borderColor: '#30363d', autoScale: false, scaleMargins: { top: 0.1, bottom: 0.1 } } });
    charts[freq].trend.priceScale('right').applyOptions({ autoScale: false });
    series[freq].trend = charts[freq].trend.addHistogramSeries({ priceLineVisible: false, lastValueVisible: true });
...
function syncTo(source, ...targets) {
    source.timeScale().subscribeVisibleLogicalRangeChange(range => {
        if (!range) return;
        targets.forEach(t => { if (t && t !== source) { try { t.timeScale().setVisibleLogicalRange(range); } catch (_) {} } });
    });
}
...
function setupResizeObserver() {
    const pairs = [
        ['chart-daily-main', charts.daily.main], ['chart-daily-rsi', charts.daily.rsi],
        ['chart-daily-macd', charts.daily.macd], ['chart-daily-trend', charts.daily.trend],
        ['chart-weekly-main', charts.weekly.main], ['chart-weekly-rsi', charts.weekly.rsi],
        ['chart-weekly-macd', charts.weekly.macd], ['chart-weekly-trend', charts.weekly.trend],
    ];
    pairs.forEach(([id, chart]) => {
        const el = document.getElementById(id);
        if (!el || !chart) return;
        new ResizeObserver(entries => {
            for (const e of entries) { const { width, height } = e.contentRect; chart.resize(width, height); }
        }).observe(el);
    });
}
...
function loadOHLCV(freq, rows) {
    ...
    series[freq].candle.setData(rows.map(r => ({ time: r.date, open: r.open, high: r.high, low: r.low, close: r.close })));
}
...
function loadIndicatorsToPanel(freq, data) {
    ...
    [7, 14, 21].forEach(p => { if (series[freq].rsi[p]) series[freq].rsi[p].setData(toLineData(data[`rsi_${p}`])); });
    ...
    if (series[freq].macdHist && data.macd_hist) {
        series[freq].macdHist.setData(data.macd_hist.map(d => {
            if (d.value == null) return { time: d.date };
            return { time: d.date, value: d.value, color: d.value >= 0 ? C.macd_hist_pos + 'cc' : C.macd_hist_neg + 'cc' };
        }));
    }
    if (series[freq].trend && data.trend_score) {
        series[freq].trend.setData(data.trend_score.map(d => {
            if (d.value == null) return { time: d.date };
            return { time: d.date, value: d.value, color: d.value > 0 ? C.trend_pos + 'cc' : d.value < 0 ? C.trend_neg + 'cc' : C.trend_zero + 'cc' };
        }));
    }
...
function addKamaPeriod(period) {
    ...
    if (kamaPeriods[p]) return null;
    const color = nextKamaColor();
    kamaPeriods[p] = { color, active: true, series_daily: null, series_weekly: null };
    if (charts.daily.main) {
        kamaPeriods[p].series_daily = charts.daily.main.addLineSeries({ color, lineWidth: 1.5, priceLineVisible: false, lastValueVisible: false });
    }
    if (charts.weekly.main) {
        kamaPeriods[p].series_weekly = charts.weekly.main.addLineSeries({ color, lineWidth: 1.5, priceLineVisible: false, lastValueVisible: false });
    }
    return color;
}
...
function removeKamaPeriod(period) {
    ...
    ['daily', 'weekly'].forEach(freq => {
        ...
        if (s && charts[freq].main) { try { charts[freq].main.removeSeries(s); } catch (_) {} }
    });

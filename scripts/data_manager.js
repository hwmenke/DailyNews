...
let _dmLibrary    = [];
let _dmSelected   = new Set();
let _dmExpanded   = new Set();
let _dmReader     = null;
let _dmRunning    = false;

async function initDataManager() {
    if (_dmLibrary.length > 0) return;
    try {
...
function _dmRenderCategories(filterText = "") {
    ...
    _dmLibrary.forEach(cat => {
        const tickers = q ? cat.tickers.filter(t => t.toLowerCase().includes(q)) : cat.tickers;
        if (q && tickers.length === 0) return;
        ...
        header.querySelector(".dm-cat-toggle").addEventListener("click", () => _dmToggleCategory(cat.id));
        ...
        if (catPartial) { const cb = header.querySelector(".dm-cat-check"); cb.indeterminate = true; }
...
function _dmToggleCategory(catId) {
    if (_dmExpanded.has(catId)) { _dmExpanded.delete(catId); } else { _dmExpanded.add(catId); }
...
function _dmToggleTicker(ticker, chip) {
    if (_dmSelected.has(ticker)) { _dmSelected.delete(ticker); chip.classList.remove("dm-chip-on"); }
    else { _dmSelected.add(ticker); chip.classList.add("dm-chip-on"); }
    _dmUpdateCount();
}
...
async function dmStartBatch() {
    ...
    if (tickers.length === 0) { _dmLogLine("⚠ No tickers selected.", "warn"); return; }
    ...
    try {
        ...
        while (true) {
            ...
            for (const part of parts) {
                ...
                if (ev.type === "start") {
                    ...
                    _dmSetProgress(pct, `${ev.index + 1} / ${tickers.length}`);
                    if (ev.ok) { okCount++; _dmLogLine(`✓ ${ev.symbol} — ${ev.msg}`, "ok"); }
                    else       { failCount++; _dmLogLine(`✗ ${ev.symbol} — ${ev.msg}`, "err"); }
                } else if (ev.type === "done") {
                    _dmSetProgress(100, `${tickers.length} / ${tickers.length}`);
                    _dmLogLine(`Done. ${ev.ok} succeeded, ${ev.failed} failed.`, ev.failed > 0 ? "warn" : "ok");
        ...
        if (err.name !== "AbortError") _dmLogLine(`Error: ${err.message}`, "err");
    } finally {
        _dmReader = null;
        _dmSetRunning(false);
    }
...
function dmAbortBatch() {
    if (_dmReader) { _dmReader.cancel("User aborted"); _dmReader = null; }
    _dmSetRunning(false);
...
function _dmShowSummary(ok, failed) {
    ...
    if (!el) return;
    el.innerHTML = `<span class="dm-sum-ok">${ok} OK</span>` + (failed > 0 ? ` <span class="dm-sum-err">${failed} failed</span>` : "");
}

function _dmClearLog() { const log = document.getElementById("dm-log"); if (log) log.innerHTML = ""; }

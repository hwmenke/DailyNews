...
function renderStats(data) {
    ...
    const kamaColors = { '10': '#3b82f6', '20': '#f97316', '50': '#a855f7' };
    ...
    const distanceChartOptions = {
        ...baseChartOpts,
        plugins: { legend: { display: true, labels: { color: '#8b949e', usePointStyle: true, boxWidth: 10 } } }
    };
    ...
    statsCharts['rsi1d'] = new Chart(document.getElementById('chart-rsi-1d'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Mean 1D Return', data: data.rsi_analysis.fwd_1d.map(d => pctValue(d.value)), backgroundColor: data.rsi_analysis.fwd_1d.map(d => pctColor(d.value)) }]
        },
    ...
    statsCharts['kamaDist1d'] = new Chart(document.getElementById('chart-kama-dist-1d'), {
        ...
        data: {
            ...
            datasets: Object.entries(data.kama_distance_analysis?.fwd_1d || {}).map(([period, points]) => ({
                label: `KAMA ${period}`, data: alignedDeciles(points),
                borderColor: kamaColors[period] || '#4facfe', backgroundColor: kamaColors[period] || '#4facfe',
                spanGaps: true, pointRadius: 3, pointHoverRadius: 5, borderWidth: 2, tension: 0.25,
            }))
        },
    ...
    statsCharts['rsi5d'] = new Chart(document.getElementById('chart-rsi-5d'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Mean 5D Return', data: data.rsi_analysis.fwd_5d.map(d => pctValue(d.value)), backgroundColor: data.rsi_analysis.fwd_5d.map(d => pctColor(d.value)) }]
        },
        options: baseChartOpts
    });
    ...
    statsCharts['dist'] = new Chart(document.getElementById('chart-dist'), {
        ...
        data: {
            ...
            datasets: [{ data: data.distribution.map(d => d.count), backgroundColor: 'rgba(79, 172, 254, 0.6)', borderColor: '#4facfe', borderWidth: 1, categoryPercentage: 1.0, barPercentage: 1.0 }]
        },
        options: { ...baseChartOpts, scales: { ...baseChartOpts.scales, x: { ...baseChartOpts.scales.x, ticks: { ...baseChartOpts.scales.x.ticks, maxRotation: 0, autoSkip: true, maxTicksLimit: 10 } } } }
    });
    ...
    statsCharts['season'] = new Chart(document.getElementById('chart-seasonality'), {
        ...
        data: {
            ...
            datasets: [{ data: data.seasonality.map(d => pctValue(d.value)), backgroundColor: data.seasonality.map(d => Number.isFinite(d.value) && d.value >= 0 ? 'rgba(34, 197, 94, 0.6)' : 'rgba(239, 68, 68, 0.6)') }]
        },
        options: baseChartOpts
    });
    ...
    statsCharts['kamaCross'] = new Chart(document.getElementById('chart-kama-cross'), {
        ...
        data: {
            ...
            datasets: [
                { label: '1D Fwd Return', data: (data.kama_cross_analysis || []).map(d => pctValue(d.fwd_1d)), backgroundColor: 'rgba(79, 172, 254, 0.65)', borderColor: '#4facfe', borderWidth: 1 },
                { label: '5D Fwd Return', data: (data.kama_cross_analysis || []).map(d => pctValue(d.fwd_5d)), backgroundColor: 'rgba(249, 115, 22, 0.65)', borderColor: '#f97316', borderWidth: 1 }
            ]
        },
    ...
    statsCharts['kamaCrossCounts'] = new Chart(document.getElementById('chart-kama-cross-counts'), {
        ...
        data: {
            labels: (data.kama_cross_analysis || []).map(d => d.label),
            datasets: [{ label: '1D Event Count', data: (data.kama_cross_analysis || []).map(d => d.count_1d), backgroundColor: (data.kama_cross_analysis || []).map(d => d.direction === 'bull' ? 'rgba(34, 197, 94, 0.6)' : 'rgba(239, 68, 68, 0.6)'), borderColor: (data.kama_cross_analysis || []).map(d => d.direction === 'bull' ? '#22c55e' : '#ef4444'), borderWidth: 1 }]
        },
        options: baseChartOpts
    });
}
...
function renderKNN(data) {
    ...
    if (featureTbody) {
        const featureLabels = {
            rsi14: 'RSI (14)', vol20_ann: 'Vol 20D Ann.', macd_hist: 'MACD Hist',
            cci_norm: 'CCI / 200', vol_ratio: 'Vol Ratio vs 20MA',
            kama_dist_10: 'Price vs KAMA10', kama_dist_20: 'Price vs KAMA20', kama_dist_50: 'Price vs KAMA50',
        };
...
function renderBacktest(data) {
    ...
    if (tbody) {
        ...
        (data.top10 || []).forEach((r, i) => {
            ...
            tr.innerHTML = `<td>${r.label}</td><td>${fmt(r.sharpe, 3)}</td><td>${fmtPct(r.ann_ret)}</td><td>${fmtPct(r.max_dd)}</td><td>${r.n_trades}</td><td>${fmtPct(r.win_rate)}</td>`;
    ...
    if (canvas && data.equity_curve && data.equity_curve.length > 0) {
        backtestEquityChart = new Chart(canvas, {
            ...
            data: {
                ...
                datasets: [
                    { label: 'Strategy',   data: data.equity_curve.map(d => d.strategy),  borderColor: '#4facfe', backgroundColor: 'rgba(79,172,254,0.08)', borderWidth: 2, pointRadius: 0, tension: 0.1, fill: true },
                    { label: 'Buy & Hold', data: data.equity_curve.map(d => d.benchmark), borderColor: '#f97316', backgroundColor: 'transparent', borderWidth: 1.5, borderDash: [5,3], pointRadius: 0, tension: 0.1 },
                ],
            },
            options: {
...
async function fetchSP500() {
    ...
    try {
        await apiFetch(`${API}/scanner/fetch`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ force: false }) });
    ...
    scannerPollTimer = setInterval(async () => {
        try {
            ...
            if (s.running) {
                ...
                if (btn) btn.disabled = false;
                const sum = s.summary || {};
                if (statusEl) statusEl.textContent = sum.error ? 'Error: ' + sum.error : `Done — ${sum.success || 0} ok, ${sum.skipped || 0} skipped, ${sum.failed || 0} failed`;

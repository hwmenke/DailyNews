...
def compute_stats(symbol: str) -> dict:
    ...
    if returns.empty:
        vol_ann = None; sharpe = None; max_dd = None; distribution = []
    else:
        ret_std = returns.std(); ret_mean = returns.mean()
    ...
    for period in KAMA_PERIODS:
        ...
        for direction, mask in (("Bull", bullish_cross), ("Bear", bearish_cross)):
            ...
            kama_cross_analysis.append({"period": period, "direction": direction.lower(),
                                        "label": f"K{period} {direction}", **summary})

    df['month'] = df.index.month
    monthly_ret = df.groupby('month')['ret_1d'].mean() * 21
    seasonality = [{"month": int(m), "value": _finite_or_none(v)} for m, v in monthly_ret.items()]

    return {
        "metrics": {"volatility": _finite_or_none(vol_ann), "sharpe": _finite_or_none(sharpe),
        ...
        "rsi_analysis": {"fwd_1d": rsi_deciles_1d, "fwd_5d": rsi_deciles_5d},
        "vol_analysis": {"fwd_1d": vol_deciles_1d},
